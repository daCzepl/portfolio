...
import ProcessAeye from "../../assets/processAeye.png";
import PlanITup from "../../assets/planItup.png";
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      // Use half of one loop's width as our max scroll
      const maxScroll = app.medias[0].widthTotal / 2;

      if (spacerRef.current) {
        spacerRef.current.style.height = `${maxScroll * 80}px`;
      }
      ScrollTrigger.getAll().forEach((trigger) => trigger.kill());

      gsap.to(app.scroll, {
        target: -maxScroll, // reduce the target shift for slower movement
        ease: "none",
        scrollTrigger: {
          trigger: containerRef.current,
          start: "top top",
          end: `+=${maxScroll * 20}`, // extend vertical scroll distance for smoother effect
          scrub: 0.8,
          pin: true,
  ...
  return <div className="h-[80vh] 2xl:h-screen flex items-center justify-center overflow-hidden " ref={containerRef} style={{ cursor: "grab" }} />;
}
